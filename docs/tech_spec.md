...
Система предназначена для управления задачами, учебными курсами и напоминаниями через Telegram-бот. Вся бизнес-логика реализуется на сервере (Django + DRF), Telegram-бот является клиентом API.
